...
class SensorComparison:
    def __init__(self):
        ...
        # Lookup table for distance correction
        ...
        self.corrected_distances = []  # LUT-corrected sensor distances
        self.corrected_deltas = []  # Delta between reference and LUT-corrected distance
        ...
        # Prompt to load lookup table
        self.load_lookup_table_prompt()
        ...
        print(f"Lookup Table: {'Loaded' if self.lut_loaded else 'Not Loaded'}")
        print(f"{'='*60}\n")
    
    def load_lookup_table_prompt(self):
        """Prompt user to load a lookup table JSON file"""
        print("\nWould you like to load a lookup table for distance correction? (y/n): ", end='')
        ...
        if response == 'y':
            ...
            filepath = filedialog.askopenfilename(
                title="Select Lookup Table JSON File",
            ...
            if filepath:
                self.load_lookup_table(filepath)
    ...
    def load_lookup_table(self, filepath):
        ...
        try:
            ...
            # Extract compiled lookup table data
            if 'compiled_positions' in data and 'compiled_distances' in data:
                self.lookup_table = {
                    'name': data.get('name', 'Unknown'),
                    'positions': data['compiled_positions'],
                    'distances': data['compiled_distances'],
                    'metadata': data.get('metadata', {})
                }
                self.lut_loaded = True
                print(f"\n[LUT] Loaded: {self.lookup_table['name']}")
                print(f"[LUT] Entries: {len(self.lookup_table['positions'])}")
                print(f"[LUT] Range: {min(self.lookup_table['distances']):.2f}mm to {max(self.lookup_table['distances']):.2f}mm\n")
            else:
                print("[LUT] Error: Invalid lookup table format (missing compiled data)")
        except Exception as e:
            print(f"[LUT] Error loading lookup table: {str(e)}")
    
    def apply_lookup_table(self, sensor_distance):
        """Apply lookup table correction to get true position from sensor distance"""
        if not self.lut_loaded or not self.lookup_table:
            return None
        
        positions = self.lookup_table['positions']
        distances = self.lookup_table['distances']
        ...
        for i in range(len(distances) - 1):
            ...
            if (d1 <= sensor_distance <= d2) or (d2 <= sensor_distance <= d1):
                ...
                if d2 != d1:
                    return p1 + (p2 - p1) * (sensor_distance - d1) / (d2 - d1)
    ...
    def get_data(self):
        ...
        if frame_type == 0x10 and payload and len(payload) >= 14:
            ...
            # Apply lookup table correction if loaded
            corrected_distance = None
            corrected_delta = 0.0
            if self.lut_loaded:
                corrected_distance = self.apply_lookup_table(distance)
                if corrected_distance is not None:
                    corrected_delta = abs(linec - corrected_distance)
            ...
            self.corrected_distances.append(corrected_distance if corrected_distance is not None else distance)
            self.corrected_deltas.append(corrected_delta)

            # Print with or without LUT correction
            if self.lut_loaded and corrected_distance is not None:
                print(f"Raw Δ: {measurement_delta:.2f}mm, LUT Δ: {corrected_delta:.2f}mm | "
                      f"{self.position_sensor_name}: {linec:.2f}mm, Raw: {distance:.2f}mm, "
                      f"Corrected: {corrected_distance:.2f}mm")
            else:
            ...
            self.write2file([distance, temp, linec, measurement_delta, distance_output, distance_output_delta, stringpot_vs_distout_delta, 
                           corrected_distance if corrected_distance is not None else distance, corrected_delta, ts])
    ...
    def plot_results(self):
        ...
        plt.plot(relative_timestamps, self.sensor_distances, label="Raw Sensor Distance", marker="o", alpha=0.7)
        if self.lut_loaded and self.corrected_distances:
            plt.plot(relative_timestamps, self.corrected_distances, label="LUT-Corrected Distance", marker="^", alpha=0.7)
        plt.plot(relative_timestamps, self.linear_encoder_positions, label=self.position_sensor_name, marker="x", linewidth=2)
        if self.distance_outputs and any(d != 0 for d in self.distance_outputs):
            plt.plot(relative_timestamps, self.distance_outputs, label="Distance Output (A2)", marker="s", alpha=0.5)
        ...
        plt.figure(figsize=(10, 5))
        plt.scatter(self.linear_encoder_positions, self.measurement_deltas, label="Raw Delta", color='r', marker=".", alpha=0.6)
        if self.lut_loaded and self.corrected_deltas:
            plt.scatter(self.linear_encoder_positions, self.corrected_deltas, label="LUT-Corrected Delta", color='g', marker=".", alpha=0.6)
        ...
        # If LUT is loaded, show improvement statistics
        if self.lut_loaded and self.corrected_deltas:
            raw_mean = np.mean(self.measurement_deltas)
            raw_std = np.std(self.measurement_deltas)
            raw_max = np.max(self.measurement_deltas)
            
            corr_mean = np.mean(self.corrected_deltas)
            corr_std = np.std(self.corrected_deltas)
            corr_max = np.max(self.corrected_deltas)
            
            print(f"\n{'='*60}")
            print("LOOKUP TABLE CORRECTION STATISTICS")
            print(f"{'='*60}")
            print(f"Raw Sensor Error:       Mean: {raw_mean:.3f}mm  Std: {raw_std:.3f}mm  Max: {raw_max:.3f}mm")
            print(f"LUT-Corrected Error:    Mean: {corr_mean:.3f}mm  Std: {corr_std:.3f}mm  Max: {corr_max:.3f}mm")
            print(f"Improvement:            Mean: {((raw_mean - corr_mean) / raw_mean * 100):.1f}%  "
                  f"Std: {((raw_std - corr_std) / raw_std * 100):.1f}%  "
                  f"Max: {((raw_max - corr_max) / raw_max * 100):.1f}%")
            print(f"{'='*60}\n")
